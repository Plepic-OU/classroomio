...
describe('courseReviewSchema', () => {
  ...
  it('accepts rating boundary values 1 and 5', () => {
    expect(courseReviewSchema.safeParse({ rating: 1, reviewText: 'Valid review text here.', courseId: VALID_UUID }).success).toBe(true);
    expect(courseReviewSchema.safeParse({ rating: 5, reviewText: 'Valid review text here.', courseId: VALID_UUID }).success).toBe(true);
  });
});
