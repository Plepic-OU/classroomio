...
> **Note:** Add `.features-gen/` to the project's `.gitignore`. This directory is generated by `bddgen` and should not be committed.
...
Given('I am on the courses page', async ({ page }) => {
  // After login, the user lands on /org/[slug] (dashboard), not /courses.
  // Extract the org path and navigate to the courses page explicitly.
  const orgMatch = page.url().match(/\/org\/[^/]+/);
  if (orgMatch) {
    await page.goto(orgMatch[0] + '/courses');
  }
...
Port `9323` is already configured in `.devcontainer/devcontainer.json` (including `appPort`, `forwardPorts`, and `portsAttributes`). The `setup.sh` script already handles Playwright browser installation and `.env.example` copying for `tests/e2e`. The relevant existing configuration:
...
The `tests/e2e` entry is already present in `pnpm-workspace.yaml`. No changes needed:

```yaml
packages:
  - apps/*
  - packages/*
  - tests/e2e        # already present
```
...
- **Test data accumulation:** The "Create a new course" scenario inserts a course named "Test Course" into the database. Repeated runs will accumulate duplicate courses. A cleanup strategy (e.g., deleting test-created courses in an `afterEach` hook, or using unique timestamped names) should be added before scaling to more scenarios.
- **Course creation form selectors are speculative:** The `input[placeholder*="title" i]` selector in the course title step needs verification against the actual `NewCourseModal` component markup.
- **i18n dependency:** Button text selectors like `"Create Course"` assume English locale. Tests will fail if the browser or user profile defaults to a different language.
