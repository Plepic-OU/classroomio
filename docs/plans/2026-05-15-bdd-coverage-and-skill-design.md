...
- **D-18 · `ROUTE_PROBES[0]` for `/login` still hardcodes 15s `waitFor` timeout while `waitForHydration` is now 30s (commit `9b2b8099`).** Same cold-cache concern as the `hydration.ts:10` bump, different call site. Narrow exposure today — no current caller of `waitForRouteHydration(page, '/login')` — but a future caller would silently get the shorter budget. *Reopen when:* any code path calls `waitForRouteHydration(page, '/login')`, OR a sweep for all hardcoded test-helper timeouts is done. Captured during step-5 validator re-run on HEAD `9b2b8099`.
