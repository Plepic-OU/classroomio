#!/usr/bin/env node
/**
 * generate-diagrams.ts — PlantUML C4 diagram generator for ClassroomIO
 *
 * Reads docs/c4/components.json (from extract-components.ts) and writes
 * L1, L2, L3-dashboard, and L3-api PlantUML C4 diagrams to docs/c4/.
 *
 * Usage (from repo root):
 *   node --import=tsx/esm .claude/skills/c4-plantuml/scripts/generate-diagrams.ts [flags]
 *
 * Flags:
...
const C4_BASE = 'https://raw.githubusercontent.com/plantuml-stdlib/C4-PlantUML/master';

function header(level: 'C4_Context' | 'C4_Container' | 'C4_Component'): string {
  return `@startuml\n!include ${C4_BASE}/${level}.puml`;
}

const FOOTER = '@enduml';

/** PlantUML node aliases must be alphanumeric + underscore */
...
function l3(app: AppOutput): string {
  ...
  const lines: string[] = [
    ...
    '',
    `Container_Boundary(${ctrAlias}, "${containerLabel}") {`,
  ];
