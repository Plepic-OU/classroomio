...
describe('FEATURE_LIMITS', () => {
  ...
  it('basic tier has correct limits', () => {
...
describe('canAccessFeature', () => {
  ...
  describe('unknown plan defaults to free', () => {
    it('cannot access custom_domain', () => expect(canAccessFeature('unknown', 'custom_domain')).toBe(false));
    it('cannot access analytics', () => expect(canAccessFeature('unknown', 'analytics')).toBe(false));
    it('cannot access api_access', () => expect(canAccessFeature('unknown', 'api_access')).toBe(false));
    it('can access certificates', () => expect(canAccessFeature('unknown', 'certificates')).toBe(true));
    it('can access community', () => expect(canAccessFeature('unknown', 'community')).toBe(true));
  });
});
...
describe('getFeatureLimit', () => {
  ...
  describe('unknown plan defaults to free', () => {
    it('returns 3 for max_courses', () => expect(getFeatureLimit('unknown_plan', 'max_courses')).toBe(3));
    it('returns 30 for max_students', () => expect(getFeatureLimit('unknown_plan', 'max_students')).toBe(30));
    it('returns false for custom_domain', () => expect(getFeatureLimit('unknown_plan', 'custom_domain')).toBe(false));
  });
});
